...
/-- The fixed-support parametric kernel used to establish smoothness of the
...
private theorem quadraticRatioSet_isCompact (g : BombieriTest) :
  ...
  apply (g.hasCompactSupport.isCompact.prod g.hasCompactSupport.isCompact).image_of_continuousOn
  exact continuousOn_fst.div continuousOn_snd fun p hp ↦ by
    exact (g.tsupport_subset hp.2).ne'

private theorem quadraticRatioSet_subset_pos (g : BombieriTest) :
    quadraticRatioSet g ⊆ Ioi 0 := by
  rintro x ⟨p, hp, rfl⟩
  have hp1 : 0 < p.1 := by simpa [positiveHalfLine] using g.tsupport_subset hp.1
  have hp2 : 0 < p.2 := by simpa [positiveHalfLine] using g.tsupport_subset hp.2
...
private theorem quadraticParametricKernel_contDiff (g : BombieriTest) :
  ...
  have harg1 : ContDiff ℝ ∞ (fun q : ℝ × ℝ ↦ -q.1 * q.2) := by fun_prop
  have harg2 : ContDiff ℝ ∞ (fun q : ℝ × ℝ ↦ -q.2) := by fun_prop
...
/-- The autocorrelation of a Bombieri test is itself a Bombieri test. -/
def bombieriQuadraticTest (g : BombieriTest) : BombieriTest :=
  TestFunction.mk
    ...
    (by simpa [positiveHalfLine] using bombieri_autocorrelation_tsupport_subset g)
