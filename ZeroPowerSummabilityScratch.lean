...
theorem ZetaZeroEnumeration.summable_inv_norm_rpowScratch
...
theorem ZetaZeroEnumeration.summable_one_sub_inv_norm_rpowScratch
    (zeros : ZetaZeroEnumeration) (p : ℝ) (hp : 1 < p) :
    Summable (fun n ↦ ‖1 - (zeros.zero n).val‖⁻¹ ^ p) := by
  simpa only [ZetaZeroEnumeration.oneSub,
    oneSubNontrivialZetaZero_val] using
      zeros.oneSub.summable_inv_norm_rpowScratch p hp
