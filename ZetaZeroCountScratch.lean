...
/-!
This file prototypes an enumeration-independent replacement for the prefix count
in `zeta_zero_density`.  The global divisor of the entire xi function records
each zero with its analytic multiplicity.  Restricting that divisor by an
indicator of a compact rectangle gives a finitely supported natural-valued
weight, so its `finsum` is an honest finite zero count.
-/
...
theorem xiZeroCount_nonneg
    (sigmaLower sigmaUpper heightLower heightUpper : ℝ) :
    0 ≤ xiZeroCount sigmaLower sigmaUpper heightLower heightUpper :=
  Nat.zero_le _
...
/-- The `finsum` count is an ordinary finite sum of analytic
multiplicities over the distinct zeros in the rectangle. -/
...
#print axioms xiDivisor_nonneg
#print axioms mem_xiDivisor_support_iff
#print axioms xiZeroWeight_hasFiniteSupport
#print axioms xiZeroCount_eq_sum
#print axioms xiZeroCount_one_sub
