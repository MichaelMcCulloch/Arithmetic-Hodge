...
/-- The real-space integrand for a weighted Mellin atom is genuinely
integrable on `(1, ∞)`. -/
...
real-space integral on `(1, ∞)`, in the ordinary inverse-power form useful
for the subsequent geometric summation. -/
...
/-- Subtracting the normalized value at `1` moves inside the atom integral
as Bombieri's bracket `f(x) + f*(x) - 2f(1)/x`. -/
...
#print axioms bombieriWeightedMellinAtom_realSpace_integrableOn
#print axioms bombieriWeightedMellinAtom_eq_realSpace
#print axioms bombieriWeightedMellinAtom_sub_eq_realSpace
