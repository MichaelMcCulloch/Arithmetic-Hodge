...
def oneSubNontrivialZetaZeroScratch
...
@[simp]
theorem oneSubNontrivialZetaZeroScratch_val
    (rho : NontrivialZetaZero) :
    (oneSubNontrivialZetaZeroScratch rho).val = 1 - rho.val := rfl
...
@[simp]
theorem oneSubNontrivialZetaZeroScratch_involutive
    (rho : NontrivialZetaZero) :
    oneSubNontrivialZetaZeroScratch
      (oneSubNontrivialZetaZeroScratch rho) = rho := by
  apply nontrivialZetaZero_ext_val_scratch
  simp

/-- `rho |-> 1-rho` as an equivalence of the nontrivial-zero subtype. -/
def oneSubNontrivialZetaZeroEquivScratch :
    NontrivialZetaZero ≃ NontrivialZetaZero where
  toFun := oneSubNontrivialZetaZeroScratch
  invFun := oneSubNontrivialZetaZeroScratch
  left_inv := oneSubNontrivialZetaZeroScratch_involutive
  right_inv := oneSubNontrivialZetaZeroScratch_involutive

@[simp]
theorem oneSubNontrivialZetaZeroScratch_eq_iff
    (rho sigma : NontrivialZetaZero) :
    oneSubNontrivialZetaZeroScratch rho = sigma ↔
      rho = oneSubNontrivialZetaZeroScratch sigma := by
  constructor
  · intro h
    rw [← h]
    exact (oneSubNontrivialZetaZeroScratch_involutive rho).symm
  · intro h
    rw [h]
    exact oneSubNontrivialZetaZeroScratch_involutive sigma

/-- Reflection preserves analytic zeta multiplicity. -/
theorem analyticOrderNatAt_riemannZeta_oneSubScratch
    (rho : NontrivialZetaZero) :
    analyticOrderNatAt riemannZeta
        (oneSubNontrivialZetaZeroScratch rho).val =
  ...
  rw [← xiZeroMultiplicity_eq_analyticOrderNatAt_riemannZeta,
    ...
    oneSubNontrivialZetaZeroScratch_val,
...
def ZetaZeroEnumeration.oneSubScratch
    (zeros : ZetaZeroEnumeration) : ZetaZeroEnumeration where
  zero k := oneSubNontrivialZetaZeroScratch (zeros.zero k)
  exhaustive rho := by
    obtain ⟨k, hk⟩ :=
      zeros.exhaustive (oneSubNontrivialZetaZeroScratch rho)
    refine ⟨k, ?_⟩
    simp [hk]
  fiberFinite rho := by
    have hset :
        {k : ℕ | oneSubNontrivialZetaZeroScratch (zeros.zero k) = rho} =
          {k : ℕ | zeros.zero k = oneSubNontrivialZetaZeroScratch rho} := by
      ext k
      exact oneSubNontrivialZetaZeroScratch_eq_iff (zeros.zero k) rho
    rw [hset]
    exact zeros.fiberFinite (oneSubNontrivialZetaZeroScratch rho)
  fiberCard rho := by
    have hset :
        {k : ℕ | oneSubNontrivialZetaZeroScratch (zeros.zero k) = rho} =
          {k : ℕ | zeros.zero k = oneSubNontrivialZetaZeroScratch rho} := by
    ...
    calc
      Nat.card {k : ℕ //
          oneSubNontrivialZetaZeroScratch (zeros.zero k) = rho} =
          Nat.card {k : ℕ //
            zeros.zero k = oneSubNontrivialZetaZeroScratch rho} :=
        Nat.card_congr (Equiv.setCongr hset)
      _ = analyticOrderNatAt riemannZeta
          (oneSubNontrivialZetaZeroScratch rho).val :=
        zeros.fiberCard (oneSubNontrivialZetaZeroScratch rho)
      _ = analyticOrderNatAt riemannZeta rho.val :=
        analyticOrderNatAt_riemannZeta_oneSubScratch rho
...
theorem ZetaZeroEnumeration.summable_one_sub_inv_norm_sq_scratch
  ...
  simpa only [ZetaZeroEnumeration.oneSubScratch,
    oneSubNontrivialZetaZeroScratch_val] using
      (zeros.oneSubScratch.summable_inv_norm_sq)
