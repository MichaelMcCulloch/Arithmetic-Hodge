...
/-- The apparent pole at `x = 1` is removable, so the integrand is Bochner
integrable on a finite right neighborhood of `1`. -/
private theorem bombieriArchIntegrableOn_Ioc_one_two (f : BombieriTest) :
  ...
  have hextIntegrable :
    ...
    (hextContinuous.integrableOn_compact isCompact_Icc).mono_set Ioc_subset_Icc_self
...
/-- Bombieri's real-space archimedean contribution. -/
noncomputable def bombieriArchTerm (f : BombieriTest) : ℂ :=
...
/-- The convergent archimedean integral, packaged as the complex-linear map
needed by a concrete Bombieri functional. -/
noncomputable def bombieriArchIntegralLinearMap : BombieriTest →ₗ[ℂ] ℂ where
