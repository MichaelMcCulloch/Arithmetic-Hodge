...
This scratch file proves the partial-fraction half of the classical
...
theorem nontrivialZetaZero_mem_hadamardZerosScratch
...
theorem hadamardPartialFractionTerm_re_nonnegScratch
...
theorem chosenZeroPartialFractionTerm_re_ge_inv_gapScratch
...
theorem hadamardZeroSum_re_ge_inv_gapScratch
    (rho : NontrivialZetaZero) {sigma : ℝ} (hsigma : 1 < sigma) :
    1 / (sigma - rho.val.re) ≤
      (∑' n, (1 /
  ...
  obtain ⟨k, hk⟩ := nontrivialZetaZero_mem_hadamardZerosScratch rho
  have hfinite := hreSumm.sum_le_tsum {k} (fun n _hn => by
    exact hadamardPartialFractionTerm_re_nonnegScratch hsigma n)
  rw [Complex.re_tsum hsumm]
  calc
    1 / (sigma - rho.val.re) ≤
        (1 / (((sigma : ℂ) + rho.val.im * I) - rho.val) +
          1 / rho.val).re :=
      chosenZeroPartialFractionTerm_re_ge_inv_gapScratch rho hsigma
...
theorem negZetaLogDeriv_re_le_chosenZeroScratch
    (rho : NontrivialZetaZero) {sigma : ℝ} (hsigma : 1 < sigma) :
    (-(deriv riemannZeta
  ...
  have hsumLower :
    ...
    simpa only [s, zeroSum] using
      hadamardZeroSum_re_ge_inv_gapScratch rho hsigma
  have heq :
...
theorem trig341_chosenZero_repulsionScratch
    (rho : NontrivialZetaZero) {sigma : ℝ} (hsigma : 1 < sigma) :
    4 / (sigma - rho.val.re) ≤
  ...
  have hpos := negZetaLogDeriv_trig_nonneg_scratch hsigma rho.val.im
  have hchosen := negZetaLogDeriv_re_le_chosenZeroScratch rho hsigma
