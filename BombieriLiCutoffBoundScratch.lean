...
def liLaplaceTailPolynomialScratch : ℕ → ℂ → ℝ → ℂ
  | 0, s, _L => 1 / s
  | m + 1, s, L =>
      (L : ℂ) ^ (m + 1) / s +
        (((m + 1 : ℕ) : ℂ) / s) *
          liLaplaceTailPolynomialScratch m s L

/-- The Mellin mass removed from the `m`-th logarithmic atom by cutting at
`epsilon`. -/
def liCutoffLogMomentScratch
    (m : ℕ) (epsilon : ℝ) (s : ℂ) : ℂ :=
  (-1 : ℂ) ^ m * (epsilon : ℂ) ^ s *
    liLaplaceTailPolynomialScratch m s (-Real.log epsilon)

/-- The total endpoint correction for Bombieri's Li polynomial. -/
def liKernelCutoffCorrectionScratch
    (n : ℕ) (epsilon : ℝ) (s : ℂ) : ℂ :=
  ∑ m ∈ Finset.range n,
    ((Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)) *
      liCutoffLogMomentScratch m epsilon s

/-- The removed atom has the exact decay factor `epsilon ^ s.re`; everything
else is a finite polynomial in `-log epsilon` and `1 / s`. -/
theorem norm_liCutoffLogMoment_eqScratch
    (m : ℕ) (epsilon : ℝ) (s : ℂ) (hepsilon : 0 < epsilon) :
    ‖liCutoffLogMomentScratch m epsilon s‖ =
      epsilon ^ s.re *
        ‖liLaplaceTailPolynomialScratch m s (-Real.log epsilon)‖ := by
  unfold liCutoffLogMomentScratch
...
theorem norm_liKernelCutoffCorrection_leScratch
    (n : ℕ) (epsilon : ℝ) (s : ℂ) (hepsilon : 0 < epsilon) :
    ‖liKernelCutoffCorrectionScratch n epsilon s‖ ≤
      epsilon ^ s.re *
        ∑ m ∈ Finset.range n,
          ‖(Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)‖ *
            ‖liLaplaceTailPolynomialScratch m s (-Real.log epsilon)‖ := by
  rw [liKernelCutoffCorrectionScratch]
  calc
    ‖∑ m ∈ Finset.range n,
        ((Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)) *
          liCutoffLogMomentScratch m epsilon s‖ ≤
        ∑ m ∈ Finset.range n,
          ‖((Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)) *
            liCutoffLogMomentScratch m epsilon s‖ := norm_sum_le _ _
    _ = ∑ m ∈ Finset.range n,
        epsilon ^ s.re *
          (‖(Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)‖ *
            ‖liLaplaceTailPolynomialScratch m s (-Real.log epsilon)‖) := by
      ...
      intro m _hm
      rw [norm_mul, norm_liCutoffLogMoment_eqScratch m epsilon s hepsilon]
      ring
    _ = epsilon ^ s.re *
        ∑ m ∈ Finset.range n,
          ‖(Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)‖ *
...
private theorem laplaceTail_integrableOnScratch
...
endpoint polynomial. -/
theorem integral_laplaceTail_eqScratch
    (m : ℕ) (s : ℂ) (hs : 0 < s.re) (L : ℝ) (hL : 0 ≤ L) :
    (∫ t : ℝ in Ioi L,
      (t : ℂ) ^ m * Complex.exp (-(s * t))) =
        Complex.exp (-(s * L)) * liLaplaceTailPolynomialScratch m s L := by
  induction m with
  | zero =>
      have hbase := integral_exp_mul_complex_Ioi
        (a := -s) (by simpa using (neg_lt_zero.mpr hs)) L
      simpa [liLaplaceTailPolynomialScratch, div_eq_mul_inv] using hbase
  | succ m ih =>
      ...
      have huv' : IntegrableOn (u * v') (Ioi L) := by
        have h := (laplaceTail_integrableOnScratch
      ...
      have hu'v : IntegrableOn (u' * v) (Ioi L) := by
        have h := (laplaceTail_integrableOnScratch m s hs L hL).const_mul
          (((m + 1 : ℕ) : ℂ))
        refine IntegrableOn.congr_fun h ?_ measurableSet_Ioi
        intro t _ht
        dsimp only [u', v, Pi.mul_apply]
      ...
      rw [hrec]
      simp only [liLaplaceTailPolynomialScratch]
      field_simp [hs0]

private theorem negExp_image_Ici_negLogScratch
...
private theorem exp_neg_mul_negLog_eq_cpowScratch
...
theorem integral_Ioc_cpow_logPow_eq_cutoffMomentScratch
    (m : ℕ) (epsilon : ℝ) (s : ℂ)
    (hepsilon0 : 0 < epsilon) (hepsilon1 : epsilon < 1)
    (hs : 0 < s.re) :
    (∫ x : ℝ in Ioc 0 epsilon,
      (x : ℂ) ^ (s - 1) * ((Real.log x) ^ m : ℝ)) =
        liCutoffLogMomentScratch m epsilon s := by
  ...
  have himage : φ '' Ici L = Ioc 0 epsilon := by
    exact negExp_image_Ici_negLogScratch hepsilon0
  ...
  calc
    ...
    _ = (-1 : ℂ) ^ m *
        (Complex.exp (-(s * L)) *
          liLaplaceTailPolynomialScratch m s L) := by
      rw [integral_laplaceTail_eqScratch m s hs L hLpos.le]
    _ = liCutoffLogMomentScratch m epsilon s := by
      rw [show Complex.exp (-(s * L)) = (epsilon : ℂ) ^ s by
        exact exp_neg_mul_negLog_eq_cpowScratch hepsilon0 s]
      dsimp only [L, liCutoffLogMomentScratch]
      ring

/-- A single logarithmic atom with the same lower cutoff as `liKernelCutoff`. -/
def liLogAtomCutoffScratch (m : ℕ) (epsilon x : ℝ) : ℂ :=
  if x ∈ Ioo epsilon 1 then ((Real.log x) ^ m : ℝ) else 0

private theorem liLogAtomCutoff_integrand_eq_indicatorScratch
    (m : ℕ) (epsilon : ℝ) (s : ℂ) (hepsilon : 0 ≤ epsilon) :
    (fun x : ℝ ↦
      (x : ℂ) ^ (s - 1) * liLogAtomCutoffScratch m epsilon x) =
...
theorem liLogAtomCutoff_mellinConvergentScratch
  ...
  rw [liLogAtomCutoff_integrand_eq_indicatorScratch m epsilon s hepsilon]
  exact hfull.indicator measurableSet_Ioi

/-- Exact cutoff formula for one logarithmic atom. -/
theorem mellin_liLogAtomCutoff_eqScratch
    (m : ℕ) (epsilon : ℝ) (s : ℂ)
    (hepsilon0 : 0 < epsilon) (hepsilon1 : epsilon < 1)
    (hs : 0 < s.re) :
    mellin (liLogAtomCutoffScratch m epsilon) s =
      mellin (liLogAtom m) s - liCutoffLogMomentScratch m epsilon s := by
  let f : ℝ → ℂ := fun x ↦
    (x : ℂ) ^ (s - 1) * liLogAtom m x
  ...
  have hlow :
      (∫ x : ℝ in Ioc 0 epsilon, f x) =
        liCutoffLogMomentScratch m epsilon s := by
    rw [← integral_Ioc_cpow_logPow_eq_cutoffMomentScratch
  ...
  have hcut :
    ...
    rw [liLogAtomCutoff_integrand_eq_indicatorScratch
  ...
  change (∫ x : ℝ in Ioi epsilon, f x) =
    (∫ x : ℝ in Ioi 0, f x) - liCutoffLogMomentScratch m epsilon s
  rw [hlow] at hsplit
  rw [eq_sub_iff_add_eq, add_comm]
  exact hsplit.symm

private theorem liKernelCutoff_eq_sum_logAtomsScratch
    (n : ℕ) (epsilon x : ℝ) :
    liKernelCutoff n epsilon x =
      ∑ m ∈ Finset.range n,
        ((Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)) *
          liLogAtomCutoffScratch m epsilon x := by
  by_cases hx : x ∈ Ioo epsilon 1
  · simp only [liKernelCutoff, liPolynomial, liLogAtomCutoffScratch,
      if_pos hx]
    push_cast
    apply Finset.sum_congr rfl
    intro m _hm
    ring
  · simp [liKernelCutoff, liLogAtomCutoffScratch, hx]

private theorem mellin_liKernelCutoff_eq_sumScratch
    (n : ℕ) (epsilon : ℝ) (s : ℂ)
    (hepsilon : 0 ≤ epsilon) (hs : 0 < s.re) :
    mellin (liKernelCutoff n epsilon) s =
      ∑ m ∈ Finset.range n,
        ((Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)) *
          mellin (liLogAtomCutoffScratch m epsilon) s := by
  unfold mellin
  simp only [smul_eq_mul]
  have hintegrand :
        ...
        fun x : ℝ ↦ ∑ m ∈ Finset.range n,
          (x : ℂ) ^ (s - 1) *
            (((Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)) *
              liLogAtomCutoffScratch m epsilon x) := by
    funext x
    rw [liKernelCutoff_eq_sum_logAtomsScratch]
    rw [Finset.mul_sum]
  rw [hintegrand]
  ...
  · apply Finset.sum_congr rfl
    ...
    calc
      (∫ x : ℝ in Ioi 0,
          (x : ℂ) ^ (s - 1) *
            (c * liLogAtomCutoffScratch m epsilon x)) =
        ∫ x : ℝ in Ioi 0,
          c * ((x : ℂ) ^ (s - 1) *
            liLogAtomCutoffScratch m epsilon x) := by
          apply setIntegral_congr_fun measurableSet_Ioi
      ...
      _ = c * ∫ x : ℝ in Ioi 0,
        ...
        MeasureTheory.integral_const_mul
          (r := c) (f := fun x : ℝ ↦
            (x : ℂ) ^ (s - 1) * liLogAtomCutoffScratch m epsilon x)
      _ = c * mellin (liLogAtomCutoffScratch m epsilon) s := by
        rfl
  · intro m _hm
    have hconv := liLogAtomCutoff_mellinConvergentScratch
...
theorem mellin_liKernelCutoff_eq_liFunction_sub_correctionScratch
    (n : ℕ) (epsilon : ℝ) (s : ℂ)
    (hepsilon0 : 0 < epsilon) (hepsilon1 : epsilon < 1)
    (hs : 0 < s.re) :
    mellin (liKernelCutoff n epsilon) s =
      liFunction n s - liKernelCutoffCorrectionScratch n epsilon s := by
  rw [mellin_liKernelCutoff_eq_sumScratch n epsilon s hepsilon0.le hs]
  simp_rw [mellin_liLogAtomCutoff_eqScratch
  ...
  have hfull :
    ...
    simpa only [liKernelCutoff, liKernel, liLogAtomCutoffScratch, liLogAtom]
      using (mellin_liKernelCutoff_eq_sumScratch
...
theorem norm_mellin_liKernelCutoff_sub_liFunction_leScratch
    (n : ℕ) (epsilon : ℝ) (s : ℂ)
    (hepsilon0 : 0 < epsilon) (hepsilon1 : epsilon < 1)
    (hs : 0 < s.re) :
    ‖mellin (liKernelCutoff n epsilon) s - liFunction n s‖ ≤
      epsilon ^ s.re *
        ∑ m ∈ Finset.range n,
          ‖(Nat.choose n (m + 1) : ℂ) / (Nat.factorial m : ℂ)‖ *
            ‖liLaplaceTailPolynomialScratch m s (-Real.log epsilon)‖ := by
  rw [mellin_liKernelCutoff_eq_liFunction_sub_correctionScratch
    n epsilon s hepsilon0 hepsilon1 hs]
  have hdiff :
      liFunction n s - liKernelCutoffCorrectionScratch n epsilon s -
          liFunction n s =
        -liKernelCutoffCorrectionScratch n epsilon s := by
    ring
  rw [hdiff, norm_neg]
  exact norm_liKernelCutoffCorrection_leScratch n epsilon s hepsilon0
