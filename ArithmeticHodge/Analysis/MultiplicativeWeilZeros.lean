/-
  Existence of an exhaustive analytic-multiplicity enumeration of the
  nontrivial Riemann-zeta zeros.

  This is kept in a separate dependency layer because it uses the xi Hadamard
  product, whereas the core multiplicative Weil definitions do not.
-/
...
private theorem nontrivialZetaZero_countable : Countable NontrivialZetaZero := by
  ...
  letI : Countable Z :=
    (show Set.Countable Z from zetaZeros_countable).to_subtype
...
/-- There exists an exhaustive sequence of all nontrivial zeta zeros whose
finite fibers have cardinality equal to analytic multiplicity. -/
