...
/-! ## Consequences of the vertical-strip Stirling estimate -/

/-- Uniform upper Stirling bound for `Gamma` on a closed vertical strip. -/
...
/-- The complex cosine grows at most exponentially in the imaginary part. -/
...
/-- In a closed vertical strip, the exponential factors in `Gamma` and
`cos (πs / 2)` cancel, leaving a polynomial bound. -/
...
/-- Uniform lower Stirling bound for `Gamma` on a closed vertical strip. -/
...
/-- Uniform lower Stirling bound for the completed-zeta real Gamma factor. -/
theorem Gammaℝ_stirling_lower_bound
    (σ₁ σ₂ : ℝ) (hσ : σ₁ ≤ σ₂) :
    ∃ A c : ℝ, 0 ≤ A ∧ 0 < c ∧
  ...
  obtain ⟨A, hA, hGamma⟩ :=
    complex_Gamma_stirling_lower_bound (σ₁ / 2) (σ₂ / 2) hhalf_strip
  let c : ℝ := Real.pi ^ (-σ₂ / 2)
...
/-- The reciprocal real Gamma factor has polynomial times exponential growth
on every closed vertical strip. -/
theorem norm_Gammaℝ_inv_stirling_upper_bound
    (σ₁ σ₂ : ℝ) (hσ : σ₁ ≤ σ₂) :
    ∃ A C : ℝ, 0 ≤ A ∧ 0 < C ∧
      ∀ s : ℂ, σ₁ ≤ s.re → s.re ≤ σ₂ → 4 ≤ |s.im| →
        ‖(Complex.Gammaℝ s)⁻¹‖ ≤
          C * |s.im| ^ A * Real.exp ((Real.pi / 4) * |s.im|) := by
  obtain ⟨A, c, hA, hc, hlower⟩ := Gammaℝ_stirling_lower_bound σ₁ σ₂ hσ
