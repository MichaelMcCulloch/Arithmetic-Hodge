...
/-!
# : dominant moduli in the full paired Bombieri--Li series
...
theorem liBase_ne_zero
    (rho : ℂ) (hrho0 : rho ≠ 0) (hrho1 : rho ≠ 1) :
    liBase rho ≠ 0 := by
  rw [liBase, show 1 - 1 / rho = (rho - 1) / rho by
    field_simp [hrho0]]
  exact div_ne_zero (sub_ne_zero.mpr hrho1) hrho0

theorem pairedLiTerm_eq_reciprocalProduct
    (n : ℕ) (rho : ℂ) (hrho0 : rho ≠ 0) (hrho1 : rho ≠ 1) :
    pairedLiTerm n rho =
      ((liBase rho) ^ n - 1) *
        (((liBase rho)⁻¹) ^ n - 1) := by
  change (1 - (liBase rho) ^ n) +
      (1 - (liBase (1 - rho)) ^ n) = _
  rw [liBase_one_sub_eq_inv rho hrho0 hrho1]
  have hw := liBase_ne_zero rho hrho0 hrho1
  have hprod : (liBase rho) ^ n *
      ((liBase rho)⁻¹) ^ n = 1 := by
    rw [← mul_pow, mul_inv_cancel₀ hw, one_pow]
  calc
    (1 - (liBase rho) ^ n) +
        (1 - ((liBase rho)⁻¹) ^ n) =
        (liBase rho) ^ n * ((liBase rho)⁻¹) ^ n -
          (liBase rho) ^ n - ((liBase rho)⁻¹) ^ n + 1 := by
      rw [hprod]
      ring
    _ = ((liBase rho) ^ n - 1) *
        (((liBase rho)⁻¹) ^ n - 1) := by ring

theorem norm_pow_mul_inv_pow_le
...
theorem norm_reciprocalPowerProduct_le
    (w : ℂ) (hw0 : w ≠ 0) (r : ℝ) (hr : 1 ≤ r)
  ...
  have hAB : ‖A * B‖ ≤ (n : ℝ) ^ 2 * r ^ (n - 1) := by
    calc
      ...
      _ ≤ ∑ _i ∈ Finset.range n,
        ...
        apply Finset.sum_le_sum
        intro j hj
        exact norm_pow_mul_inv_pow_le w hw0 r hr hw hwinv
...
def reciprocalCoefficient (w : ℂ) : ℝ :=
  ‖w - 1‖ * ‖w⁻¹ - 1‖

def zeroLiBase (zeros : ZetaZeroEnumeration) (k : ℕ) : ℂ :=
  liBase (zeros.zero k).val

def zeroReciprocalRadius
    (zeros : ZetaZeroEnumeration) (k : ℕ) : ℝ :=
  reciprocalRadius (zeroLiBase zeros k)

def zeroReciprocalCoefficient
    (zeros : ZetaZeroEnumeration) (k : ℕ) : ℝ :=
  reciprocalCoefficient (zeroLiBase zeros k)

theorem nontrivialZetaZero_val_ne_zero
...
theorem nontrivialZetaZero_val_ne_one
...
theorem reciprocalCoefficient_liBase_eq
    (rho : ℂ) (hrho0 : rho ≠ 0) (hrho1 : rho ≠ 1) :
    reciprocalCoefficient (liBase rho) =
      ‖rho‖⁻¹ * ‖1 - rho‖⁻¹ := by
  have hdirect : ‖liBase rho - 1‖ = ‖rho‖⁻¹ := by
    rw [show liBase rho - 1 = -(1 / rho) by
      unfold liBase
      ring]
    simp
  have hreflected : ‖(liBase rho)⁻¹ - 1‖ = ‖1 - rho‖⁻¹ := by
    rw [← liBase_one_sub_eq_inv rho hrho0 hrho1]
    rw [show liBase (1 - rho) - 1 = -(1 / (1 - rho)) by
      unfold liBase
      ring]
    simp
  simp only [reciprocalCoefficient, hdirect, hreflected]

theorem summable_zeroReciprocalCoefficient
    (zeros : ZetaZeroEnumeration) :
    Summable (zeroReciprocalCoefficient zeros) := by
  ...
  intro k
  exact (reciprocalCoefficient_liBase_eq
    (zeros.zero k).val
    (nontrivialZetaZero_val_ne_zero (zeros.zero k))
    (nontrivialZetaZero_val_ne_one (zeros.zero k))).symm

theorem finite_superlevel_of_summable_sq
...
theorem norm_liBase_le_one_add_inv_norm (rho : ℂ) :
    ‖liBase rho‖ ≤ 1 + ‖rho‖⁻¹ := by
  unfold liBase
  calc
...
theorem norm_liBase_inv_le_one_add_reflected_inv_norm
    (rho : ℂ) (hrho0 : rho ≠ 0) (hrho1 : rho ≠ 1) :
    ‖(liBase rho)⁻¹‖ ≤ 1 + ‖1 - rho‖⁻¹ := by
  rw [← liBase_one_sub_eq_inv rho hrho0 hrho1]
  exact norm_liBase_le_one_add_inv_norm (1 - rho)

theorem zeroReciprocalRadius_le
    (zeros : ZetaZeroEnumeration) (k : ℕ) :
    zeroReciprocalRadius zeros k ≤
      1 + max ‖(zeros.zero k).val‖⁻¹
        ‖1 - (zeros.zero k).val‖⁻¹ := by
  unfold zeroReciprocalRadius reciprocalRadius zeroLiBase
  apply max_le
  · exact (norm_liBase_le_one_add_inv_norm (zeros.zero k).val).trans
      (add_le_add_right (le_max_left _ _) 1)
  · exact (norm_liBase_inv_le_one_add_reflected_inv_norm
      (zeros.zero k).val
      (nontrivialZetaZero_val_ne_zero (zeros.zero k))
      (nontrivialZetaZero_val_ne_one (zeros.zero k))).trans
      (add_le_add_right (le_max_right _ _) 1)

theorem finite_zeroReciprocalRadius_superlevel
    (zeros : ZetaZeroEnumeration) {r : ℝ} (hr : 1 < r) :
    {k : ℕ | r ≤ zeroReciprocalRadius zeros k}.Finite := by
...
theorem exists_large_finite_reciprocalProduct_sum_re_le
  ...
  calc
    (∑ i : ι, Complex.reCLM
      ...
      rw [reciprocalPowerProduct_eq z hz0 n]
...
theorem exists_large_finite_reciprocalProduct_sum_re_negative
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (θ : ι → UnitAddCircle) {M : ℝ} (hM : 1 < M) (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧
      (∑ i : ι,
  ...
  obtain ⟨n, hNn, hn⟩ :=
    exists_large_finite_reciprocalProduct_sum_re_le θ hM N
...
theorem exists_large_finite_equal_norm_reciprocalProduct_sum_re_le
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (z : ι → ℂ)
  ...
  obtain ⟨n, hNn, hn⟩ :=
    exists_large_finite_reciprocalProduct_sum_re_le
      θ hM N
  refine ⟨n, hNn, ?_⟩
  simpa only [← hbase] using hn

theorem exists_large_finite_equal_norm_reciprocalProduct_sum_re_negative
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (z : ι → ℂ)
    {M : ℝ} (hM : 1 < M) (hnorm : ∀ i, ‖z i‖ = M)
    (N : ℕ) :
  ...
  obtain ⟨n, hNn, hn⟩ :=
    exists_large_finite_equal_norm_reciprocalProduct_sum_re_le
      z hM hnorm N
  refine ⟨n, hNn, hn.trans_lt ?_⟩
  have hcard : 0 < (Fintype.card ι : ℝ) := by
    exact_mod_cast Fintype.card_pos
...
theorem exists_large_finite_equal_reciprocalRadius_sum_re_le
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (w : ι → ℂ)
    {M : ℝ} (hM : 1 < M)
    (hradius : ∀ i, reciprocalRadius (w i) = M)
    (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧
  ...
  have hznorm (i : ι) : ‖z i‖ = M := by
    ...
    · have hrad := hradius i
      unfold reciprocalRadius at hrad
      have hinv : ‖(w i)⁻¹‖ = M := by
...
theorem exists_large_finite_equal_reciprocalRadius_sum_re_negative
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (w : ι → ℂ)
    {M : ℝ} (hM : 1 < M)
    (hradius : ∀ i, reciprocalRadius (w i) = M)
  ...
  obtain ⟨n, hNn, hn⟩ :=
    exists_large_finite_equal_reciprocalRadius_sum_re_le
      w hM hradius N
  refine ⟨n, hNn, hn.trans_lt ?_⟩
  have hcard : 0 < (Fintype.card ι : ℝ) := by
    exact_mod_cast Fintype.card_pos
...
def zeroPairedLiTerm
    (zeros : ZetaZeroEnumeration) (n k : ℕ) : ℂ :=
  pairedLiTerm n (zeros.zero k).val

theorem zeroPairedLiTerm_eq_reciprocalProduct
    (zeros : ZetaZeroEnumeration) (n k : ℕ) :
    zeroPairedLiTerm zeros n k =
      ((zeroLiBase zeros k) ^ n - 1) *
        (((zeroLiBase zeros k)⁻¹) ^ n - 1) := by
  exact pairedLiTerm_eq_reciprocalProduct n (zeros.zero k).val
    (nontrivialZetaZero_val_ne_zero (zeros.zero k))
    (nontrivialZetaZero_val_ne_one (zeros.zero k))

theorem summable_zeroPairedLiTerm
    (zeros : ZetaZeroEnumeration) (n : ℕ) :
    Summable (zeroPairedLiTerm zeros n) := by
  simpa only [zeroPairedLiTerm, pairedLiTerm] using
    zeros.liFunction_paired_summable n

theorem norm_zeroPairedLiTerm_le
    (zeros : ZetaZeroEnumeration) {r : ℝ} (hr : 1 ≤ r)
    (n k : ℕ) (hk : zeroReciprocalRadius zeros k ≤ r) :
    ‖zeroPairedLiTerm zeros n k‖ ≤
      (n : ℝ) ^ 2 * r ^ (n - 1) *
        zeroReciprocalCoefficient zeros k := by
  let w := zeroLiBase zeros k
  have hw0 : w ≠ 0 := liBase_ne_zero (zeros.zero k).val
    (nontrivialZetaZero_val_ne_zero (zeros.zero k))
    (nontrivialZetaZero_val_ne_one (zeros.zero k))
  have hw : ‖w‖ ≤ r := by
  ...
  rw [zeroPairedLiTerm_eq_reciprocalProduct]
  simpa only [w, zeroReciprocalCoefficient,
    reciprocalCoefficient, mul_assoc] using
      norm_reciprocalPowerProduct_le
        w hw0 r hr hw hwinv n

theorem norm_tsum_zeroPairedLiTerm_compl_le
    (zeros : ZetaZeroEnumeration) (D : Finset ℕ)
    {r : ℝ} (hr : 1 ≤ r)
    (htail : ∀ k ∉ D, zeroReciprocalRadius zeros k ≤ r)
    (n : ℕ) :
    ‖∑' k : {k : ℕ // k ∉ D}, zeroPairedLiTerm zeros n k‖ ≤
      ((n : ℝ) ^ 2 * r ^ (n - 1)) *
        ∑' k : ℕ, zeroReciprocalCoefficient zeros k := by
  let A : ℝ := (n : ℝ) ^ 2 * r ^ (n - 1)
  have hA0 : 0 ≤ A := by
    dsimp only [A]
    positivity
  have hcoeff := summable_zeroReciprocalCoefficient zeros
  have hcoeffCompl : Summable (fun k : {k : ℕ // k ∉ D} ↦
      zeroReciprocalCoefficient zeros k) := by
    simpa only [Function.comp_apply] using
      hcoeff.subtype {k : ℕ | k ∉ D}
  have hmajor : Summable (fun k : {k : ℕ // k ∉ D} ↦
      A * zeroReciprocalCoefficient zeros k) :=
    hcoeffCompl.mul_left A
  have hbound (k : {k : ℕ // k ∉ D}) :
      ‖zeroPairedLiTerm zeros n k‖ ≤
        A * zeroReciprocalCoefficient zeros k := by
    exact norm_zeroPairedLiTerm_le zeros hr n k
      (htail k k.property)
  have habs : Summable (fun k : {k : ℕ // k ∉ D} ↦
      ‖zeroPairedLiTerm zeros n k‖) := by
    exact hmajor.of_nonneg_of_le (fun _k ↦ norm_nonneg _) hbound
  have hcomplLe :
      (∑' k : {k : ℕ // k ∉ D},
        zeroReciprocalCoefficient zeros k) ≤
      ∑' k : ℕ, zeroReciprocalCoefficient zeros k := by
    have hsplit :
        (∑ k ∈ D, zeroReciprocalCoefficient zeros k) +
          (∑' k : {k : ℕ // k ∉ D},
            zeroReciprocalCoefficient zeros k) =
          ∑' k : ℕ, zeroReciprocalCoefficient zeros k := by
    ...
    exact le_add_of_nonneg_left
      (Finset.sum_nonneg fun _k _hk ↦ by
        unfold zeroReciprocalCoefficient reciprocalCoefficient
        positivity)
  calc
    ‖∑' k : {k : ℕ // k ∉ D}, zeroPairedLiTerm zeros n k‖ ≤
        ∑' k : {k : ℕ // k ∉ D},
    ...
    _ = A * (∑' k : {k : ℕ // k ∉ D},
        zeroReciprocalCoefficient zeros k) := tsum_mul_left
    _ ≤ A * (∑' k : ℕ,
        zeroReciprocalCoefficient zeros k) :=
      mul_le_mul_of_nonneg_left hcomplLe hA0

theorem eventually_polynomial_subdominant_lt
    (C d : ℝ) (hC : 0 ≤ C) (hd : 0 < d)
...
theorem exists_large_zeroPaired_tsum_re_negative_of_gap
    (zeros : ZetaZeroEnumeration) (D : Finset ℕ) (hD : D.Nonempty)
    {M r : ℝ} (hM : 1 < M) (hr : 1 ≤ r) (hrM : r < M)
    (hdominant : ∀ k ∈ D, zeroReciprocalRadius zeros k = M)
    (htail : ∀ k ∉ D, zeroReciprocalRadius zeros k ≤ r)
    (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧
      (∑' k : ℕ, zeroPairedLiTerm zeros n k).re < 0 := by
  let C : ℝ := ∑' k : ℕ, zeroReciprocalCoefficient zeros k
  let d : ℝ := D.card
  have hC : 0 ≤ C := by
    dsimp only [C]
  ...
  have hevent := eventually_polynomial_subdominant_lt
    C d hC hd hr hrM
  obtain ⟨N₀, hN₀⟩ := eventually_atTop.mp hevent
  letI : Nonempty D := Finset.nonempty_coe_sort.mpr hD
  obtain ⟨n, hn, hdominantBoundSubtype⟩ :=
    exists_large_finite_equal_reciprocalRadius_sum_re_le
      (fun i : D ↦ zeroLiBase zeros i) hM
      (fun i ↦ by
        simpa only [zeroReciprocalRadius] using
  ...
  have hsmall :
    ...
    hN₀ n hN₀n
  have hfiniteSumEq :
      (∑ k ∈ D, zeroPairedLiTerm zeros n k) =
        ∑ i : D, ((zeroLiBase zeros i) ^ n - 1) *
          (((zeroLiBase zeros i)⁻¹) ^ n - 1) := by
    calc
      (∑ k ∈ D, zeroPairedLiTerm zeros n k) =
          ∑ i : D, zeroPairedLiTerm zeros n i := by
        exact Finset.sum_subtype D (fun _k ↦ Iff.rfl)
          (zeroPairedLiTerm zeros n)
      _ = ∑ i : D, ((zeroLiBase zeros i) ^ n - 1) *
          (((zeroLiBase zeros i)⁻¹) ^ n - 1) := by
        apply Finset.sum_congr rfl
        intro i _hi
        exact zeroPairedLiTerm_eq_reciprocalProduct zeros n i
  have hdominantBound :
      (∑ k ∈ D, zeroPairedLiTerm zeros n k).re ≤
        -d * M ^ n / 4 := by
    rw [hfiniteSumEq]
  ...
  have htailNorm : ‖tail‖ ≤
    ...
    simpa only [tail, C] using
      norm_tsum_zeroPairedLiTerm_compl_le zeros D hr htail n
  have htailSmall : ‖tail‖ < d * M ^ n / 4 := by
    ...
    calc
      ...
      _ < d * M ^ n / 4 := hsmall
  have hseries := summable_zeroPairedLiTerm zeros n
  have hsplit :
      (∑ k ∈ D, zeroPairedLiTerm zeros n k) + tail =
        ∑' k : ℕ, zeroPairedLiTerm zeros n k := by
...
theorem exists_dominant_finset_gap
    (q : ℕ → ℝ)
...
theorem exists_large_zeroPaired_tsum_re_negative_of_radius_gt_one
    (zeros : ZetaZeroEnumeration)
    (hoff : ∃ k : ℕ, 1 < zeroReciprocalRadius zeros k)
    (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧
      (∑' k : ℕ, zeroPairedLiTerm zeros n k).re < 0 := by
  obtain ⟨D, M, r, hD, hM, hr, hrM, hdominant, htail⟩ :=
    exists_dominant_finset_gap
      (zeroReciprocalRadius zeros)
      (fun hs ↦ finite_zeroReciprocalRadius_superlevel zeros hs)
      hoff
  exact exists_large_zeroPaired_tsum_re_negative_of_gap
    zeros D hD hM hr hrM hdominant htail N

theorem not_RH_exists_zeroReciprocalRadius_gt_one
    (zeros : ZetaZeroEnumeration) (hnot : ¬ RiemannHypothesis) :
    ∃ k : ℕ, 1 < zeroReciprocalRadius zeros k := by
...
theorem not_RH_exists_large_zeroPaired_tsum_re_negative
    (zeros : ZetaZeroEnumeration) (hnot : ¬ RiemannHypothesis) (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧
      (∑' k : ℕ, zeroPairedLiTerm zeros n k).re < 0 := by
  exact exists_large_zeroPaired_tsum_re_negative_of_radius_gt_one
    zeros (not_RH_exists_zeroReciprocalRadius_gt_one zeros hnot) N

theorem not_RH_exists_large_liFunction_paired_tsum_re_negative
    (zeros : ZetaZeroEnumeration) (hnot : ¬ RiemannHypothesis) (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧
      (∑' k : ℕ,
  ...
  simpa only [zeroPairedLiTerm, pairedLiTerm] using
    not_RH_exists_large_zeroPaired_tsum_re_negative zeros hnot N

theorem RH_of_liFunction_paired_tsum_re_nonnegative
    (zeros : ZetaZeroEnumeration)
    (hnonnegative : ∀ n : ℕ, 0 ≤
      (∑' k : ℕ,
        (liFunction n (zeros.zero k).val +
