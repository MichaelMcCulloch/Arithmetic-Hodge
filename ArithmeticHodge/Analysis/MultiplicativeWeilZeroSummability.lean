/-
  Absolute summability of Mellin transforms over the zeta-zero divisor.

  Uniform Fourier decay in the critical strip supplies an inverse-square
  Mellin bound.  The finite-order xi divisor supplies the matching
  multiplicity-weighted inverse-square summability, which transfers to every
  exact-multiplicity `ZetaZeroEnumeration`.
-/
...
private theorem norm_logarithmicPullback_le
  ...
  simp only [iteratedDeriv_zero, Real.norm_eq_abs,
    abs_of_pos (Real.exp_pos _)] at hweight
...
private theorem exists_uniform_logarithmicPullback_integral_bounds
    (f : BombieriTest) :
    ∃ C0 C2 : ℝ, 0 ≤ C0 ∧ 0 ≤ C2 ∧
  ...
  · change (∫ u : ℝ, ‖H u‖) ≤ C0
    have hbound0 : ∀ u ∈ Metric.closedBall (0 : ℝ) R, ‖H u‖ ≤ D0 := by
      intro u hu
      exact norm_logarithmicPullback_le f hsigma0 hsigma1 hu
...
/-- Mellin transforms of a Bombieri test admit a uniform inverse-square bound
at every nontrivial zeta zero. -/
theorem BombieriTest.exists_mellin_norm_le_inv_norm_sq
    (f : BombieriTest) :
  ...
  rw [bombieriMellin_eq_fourier_logarithmicPullback]
  change ‖𝓕 (H : ℝ → ℂ) w‖ ≤ _
  have hdiv : ‖𝓕 (H : ℝ → ℂ) w‖ ≤
      (C0 + C2) / ‖rho.val‖ ^ 2 := by
...
zeta is analytic and nonvanishing, so their zero multiplicities agree. -/
...
/-- Every exact-multiplicity zeta-zero enumeration inherits inverse-square
summability from the xi divisor. -/
theorem ZetaZeroEnumeration.summable_inv_norm_sq
...
/-- The zero-side Mellin series in Bombieri's explicit formula is absolutely
summable for every compactly supported smooth multiplicative test. -/
theorem ZetaZeroEnumeration.mellin_summable
    (zeros : ZetaZeroEnumeration) (f : BombieriTest) :
    Summable (fun n ↦ mellin (f : ℝ → ℂ) (zeros.zero n).val) := by
  obtain ⟨C, _hC, hbound⟩ := f.exists_mellin_norm_le_inv_norm_sq
  have hinv := zeros.summable_inv_norm_sq
  have hnorm : Summable
