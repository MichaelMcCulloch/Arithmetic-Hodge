/-
  The real-space evaluation of Bombieri's gamma-factor integral.

  The corrected weighted Mellin atoms sum geometrically to the convergent
  archimedean kernel.  Reindexing the remaining harmonic correction supplies
  `log 4`; adjoining the `-log π` term then gives exactly Bombieri's
  archimedean contribution in equation (2.8).
-/
...
/-- Bombieri's normalized real-part digamma integral, together with the
`-log π` contribution from the real gamma factor, is exactly the convergent
real-space archimedean term in equation (2.8). -/
theorem bombieri_digamma_integral_sub_log_pi_eq_archTerm
    (f : BombieriTest) :
    ((1 / (2 * Real.pi) : ℝ) : ℂ) *
        (∫ v : ℝ,
