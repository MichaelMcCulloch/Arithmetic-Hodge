/-
  Fiberwise summation for the exact-multiplicity zeta-zero enumeration.

  The natural sigma-fiber equivalence partitions the enumeration by its
  distinct nontrivial zeros.  Each finite fiber then contributes analytic
  multiplicity, with summability transported before any `tsum` rewrite.
-/
...
/-- Summability along an exact-multiplicity enumeration implies summability
over distinct nontrivial zeros weighted by analytic multiplicity. -/
...
/-- Reindex an exact-multiplicity zero enumeration as a sum over distinct
nontrivial zeros, with analytic multiplicity as coefficient. -/
