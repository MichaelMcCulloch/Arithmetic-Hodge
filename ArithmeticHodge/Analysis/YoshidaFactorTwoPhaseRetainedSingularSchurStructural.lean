...
positive singular Gram.  This module spends the same fraction of that Gram
on both diagonals and removes it from the mixed row before asking for a
scalar complement estimate.  This is the lossless operator-valued
replacement for bounding the entire survivor by a coarse scalar reserve.
-/

/-- If the low and tail phases retain the same fraction of the singular
Gram, it is enough to control only the mixed row left after subtracting that
singular polarization. -/
theorem factorTwoEndpointLowTailMixed_sq_le_of_retained_singular
  ...
  let x := theta * lowSingular
  let y := theta * tailSingular
  let z := theta * pole
  ...
  have hx : 0 ≤ x := by
    exact mul_nonneg htheta hLowSingular
  have hy : 0 ≤ y := by
    exact mul_nonneg htheta hTailSingular
  ...
  have hz : z ^ 2 ≤ x * y := by
    have hscaled := mul_le_mul_of_nonneg_left hpole (sq_nonneg theta)
    dsimp only [z, x, y]
    nlinarith
