...
/-- Every nontrivial zeta zero lies in the open critical strip. -/
theorem nontrivial_zeta_zero_re (s : ℂ)
...
/-! ### Enumeration-independent zero-count replacement

The former `zeta_zero_density` declaration was removed because its statement
filtered an arbitrary, non-height-ordered prefix of `hadamardZeros`, used the
one-sided Riemann--von Mangoldt main term for a two-sided count, and attempted
to encode an unwrapped Gamma phase with the bounded principal logarithm.

`ZetaZeroCount.lean` now provides the sound multiplicity-aware replacement
`xiCriticalStripZeroCount_eventually_le_rpow`: for every `alpha > 1`, the
number of xi zeros in the symmetric critical-strip rectangle is eventually
`O(T^alpha)`, proved from Jensen's formula and the order of xi.  A sharp
Riemann--von Mangoldt asymptotic still requires an oriented argument-principle
contour and a continuous unwrapped phase.
-/
-- ============================================================
