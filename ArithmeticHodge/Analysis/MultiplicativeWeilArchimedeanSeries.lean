/-
  The odd geometric expansion of Bombieri's real-space archimedean kernel.

  On `1 < x`, the odd powers of `x⁻¹` sum to `x / (x² - 1)`.  The
  numerator's removable zero at `x = 1` makes the resulting norm series
  integrable, so Bochner dominated convergence justifies exchanging its sum
  with the integral.  This is the geometric-series step in Bombieri (2.8).
-/
...
/-- The odd geometric expansion of Bombieri's archimedean kernel may be
integrated term by term on `(1, ∞)`.  The `HasSum` form records genuine
summability for subsequent reindexing. -/
...
/-- The `tsum` form of `hasSum_integral_bombieriArchSeriesTerm`. -/
