...
/-- Endpoint-potential source generated by one centered Legendre mode. -/
def cutoffEightPotentialLegendreSource (m : ℕ) (x : ℝ) : ℝ :=
  yoshidaEndpointPotential x *
    (centeredShiftedLegendreReal m).eval x

private theorem measurable_cutoffEightPotentialLegendreSource (m : ℕ) :
    Measurable (cutoffEightPotentialLegendreSource m) := by
  unfold cutoffEightPotentialLegendreSource yoshidaEndpointPotential
  exact ((Real.measurable_log.comp
    (measurable_const.sub (measurable_id.pow_const 2))).neg.div_const 2).mul
      (centeredShiftedLegendreReal m).continuous.measurable

private theorem memLp_cutoffEightPotentialLegendreSource_restrict (m : ℕ) :
    MemLp (cutoffEightPotentialLegendreSource m) 2
      (volume.restrict (Ioc (-1 : ℝ) 1)) := by
  simpa only [cutoffEightPotentialLegendreSource] using
    memLp_endpointPotential_mul_continuous
      (fun x : ℝ ↦ (centeredShiftedLegendreReal m).eval x)
      (centeredShiftedLegendreReal m).continuous

private theorem memLp_centeredPullback_cutoffEightPotentialLegendreSource
    (m : ℕ) :
    MemLp (fun t : unitInterval ↦
      centeredPullback (cutoffEightPotentialLegendreSource m) (t : ℝ)) 2 :=
  centeredPullback_memLp_two_of_memLp_restrict
    (cutoffEightPotentialLegendreSource m)
    (measurable_cutoffEightPotentialLegendreSource m)
    (memLp_cutoffEightPotentialLegendreSource_restrict m)

/-- Canonical unit-interval `L²` class of one endpoint-potential Legendre
source. -/
def cutoffEightPotentialLegendreSourceL2 (m : ℕ) : UnitIntervalL2 :=
  (memLp_centeredPullback_cutoffEightPotentialLegendreSource m).toLp
    (fun t : unitInterval ↦
      centeredPullback (cutoffEightPotentialLegendreSource m) (t : ℝ))

/-- Every Hilbert coefficient of an endpoint-potential source is its
centered potential pairing with the exact affine and normalization factors. -/
theorem repr_cutoffEightPotentialLegendreSourceL2 (m n : ℕ) :
    shiftedLegendreHilbertBasis.repr
  ...
  calc
    (∫ t : unitInterval,
        centeredPullback (cutoffEightPotentialLegendreSource m) (t : ℝ) *
          (shiftedLegendreReal n).eval (t : ℝ)) =
        ∫ t : ℝ in 0..1,
          centeredPullback (cutoffEightPotentialLegendreSource m) t *
            (shiftedLegendreReal n).eval t := by
      exact integral_unitInterval_eq_intervalIntegral
        (fun t ↦ centeredPullback (cutoffEightPotentialLegendreSource m) t *
          (shiftedLegendreReal n).eval t)
    _ = ∫ t : ℝ in 0..1,
        yoshidaEndpointPotential (2 * t - 1) *
          (centeredShiftedLegendreReal m).eval (2 * t - 1) *
          (centeredShiftedLegendreReal n).eval (2 * t - 1) := by
      apply intervalIntegral.integral_congr
      intro t _ht
      dsimp only
      unfold centeredPullback cutoffEightPotentialLegendreSource
      have hncenter :
          (centeredShiftedLegendreReal n).eval (2 * t - 1) =
            (shiftedLegendreReal n).eval t := by
        rw [eval_centeredShiftedLegendreReal]
    ...
    _ = (1 / 2 : ℝ) *
        (∫ x : ℝ in -1..1,
          yoshidaEndpointPotential x *
            (centeredShiftedLegendreReal m).eval x *
            (centeredShiftedLegendreReal n).eval x) := by
      exact integral_comp_two_mul_sub_one (fun x ↦
        yoshidaEndpointPotential x *
          (centeredShiftedLegendreReal m).eval x *
          (centeredShiftedLegendreReal n).eval x)
    _ = _ := by ring
