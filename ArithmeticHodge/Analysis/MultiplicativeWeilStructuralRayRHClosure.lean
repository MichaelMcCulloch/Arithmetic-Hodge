...
any analytic premise.  Length four is the canonical even quotient-tail
Loewner inequality plus the exact odd corrected determinant.  Length five is the pair of
...
def ArithmeticHodgeStructuralRayCertificate : Prop :=
  FourCellEvenCanonicalSchurResidualLoewner ∧
...
theorem riemannHypothesis_of_structuralRayCertificate
  ...
  exact riemannHypothesis_of_four_five_and_tailPositiveRay
    zeros
    (realFiniteBlockProductionNonnegativeAtLength_four_of_evenCanonicalLoewner_and_oddCorrectedDefect
      hcert.1 hcert.2.1)
