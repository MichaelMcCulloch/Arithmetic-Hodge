...
/-- Uniform Dirichlet-series bound in a right half-plane. -/
private theorem riemannZeta_dirichlet_uniform_bound (a : ℝ) (ha : 1 < a) :
...
/-- Polynomial growth on a closed strip strictly left of the critical strip. -/
private theorem riemannZeta_polynomial_bound_of_re_le_neg
    (σ₁ σ₂ : ℝ) (hσ : σ₁ ≤ σ₂) (hσ₂ : σ₂ < 0) :
    ∃ A C : ℝ, 0 ≤ A ∧ 0 < C ∧
  ...
  have hq_re_hi : q.re ≤ 1 - σ₁ := by
    dsimp [q]
    linarith
  have hq_im_abs : |q.im| = |s.im| := by simp [q]
...
/-- Entire regularization of `s (s - 1) ζ(s)` using `Λ₀` and `Γℝ⁻¹`. -/
private noncomputable def zetaOrderRegularized (s : ℂ) : ℂ :=
  (s * (s - 1) * completedRiemannZeta₀ s + 1) * (Complex.Gammaℝ s)⁻¹

private theorem differentiable_zetaOrderRegularized :
...
private theorem zetaOrderRegularized_eq_of_im_ne_zero
    (s : ℂ) (hs : s.im ≠ 0) :
...
private theorem norm_completedRiemannZeta₀_le_rpow
    (r : ℝ) (hr : 26 ≤ r) (s : ℂ) (hs : ‖s‖ ≤ r) :
    ‖completedRiemannZeta₀ s‖ ≤ (r + 10) ^ (r + 10) := by
  exact (norm_completedRiemannZeta₀_le_gamma r (by linarith) s hs).trans
    (four_mul_Gamma_le_completedZeta_scale r (by linarith))

private theorem norm_zetaOrderNumerator_le_rpow
    (r : ℝ) (hr : 26 ≤ r) (s : ℂ) (hs : ‖s‖ ≤ r) :
    ‖s * (s - 1) * completedRiemannZeta₀ s + 1‖ ≤
      (r + 10) ^ (r + 13) := by
  let R : ℝ := r + 10
  ...
  have hLambda : ‖completedRiemannZeta₀ s‖ ≤ L := by
    simpa [R, L] using norm_completedRiemannZeta₀_le_rpow r hr s hs
  have hs₁ : ‖s - 1‖ ≤ r + 1 := by
    calc
...
private theorem norm_zetaOrderRegularized_le_exp_quadratic
    (σ₁ σ₂ : ℝ) (hσ : σ₁ ≤ σ₂) :
    ∃ D K : ℝ, 0 < D ∧ 0 < K ∧
      ∀ s : ℂ, σ₁ ≤ s.re → s.re ≤ σ₂ → 4 ≤ |s.im| →
        ‖zetaOrderRegularized s‖ ≤ D * Real.exp (K * |s.im| ^ 2) := by
  obtain ⟨A, C, hA, hC, hGamma⟩ :=
    norm_Gammaℝ_inv_stirling_upper_bound σ₁ σ₂ hσ
  let M : ℝ := max |σ₁| |σ₂|
  let Q : ℝ := M + 35
  ...
  have hnum_raw := norm_zetaOrderNumerator_le_rpow r hr s hs_norm
...
private theorem zetaOrderRegularized_vertical_strip_growth
    (a b : ℝ) (hab : a < b) :
    ∃ c < Real.pi / (b - a), ∃ B,
  ...
  obtain ⟨D, K, hD, hK, hquad⟩ :=
    norm_zetaOrderRegularized_le_exp_quadratic a b hab.le
...
private theorem zetaOrderRegularized_bounded_on_short_vertical_line
    (x T : ℝ) :
...
private theorem norm_le_linear_abs_im_on_vertical_line
...
private noncomputable def zetaOrderPLNormalized (p : ℝ) (N : ℕ) (s : ℂ) : ℂ :=
  zetaOrderRegularized s * (s - (p : ℂ))⁻¹ ^ N

private theorem zetaOrderPLNormalized_diffContOnCl
    (p a b : ℝ) (N : ℕ) (hp : p < a) :
...
private theorem zetaOrderPLNormalized_vertical_strip_growth
    ...
    ∃ c < Real.pi / (b - a), ∃ B,
      zetaOrderPLNormalized p N =O[
        comap (_root_.abs ∘ Complex.im) atTop ⊓
          Filter.principal (Complex.re ⁻¹' Set.Ioo a b)]
        fun z => Real.exp (B * Real.exp (c * |z.im|)) := by
  obtain ⟨c, hc, B, hG⟩ := zetaOrderRegularized_vertical_strip_growth a b hab
  refine ⟨c, hc, B, ?_⟩
...
private theorem norm_zetaOrderPLNormalized_on_large_vertical_line
  ...
  have hs_norm : ‖s‖ ≤ k₀ * T := by
    simpa [k₀, T] using norm_le_linear_abs_im_on_vertical_line x s hsx ht
  have hs_sub_norm : ‖s - 1‖ ≤ k₁ * T := by
    have hre : (s - 1).re = x - 1 := by simp [hsx]
    have him_abs : |(s - 1).im| = |s.im| := by simp
    have := norm_le_linear_abs_im_on_vertical_line
...
private theorem norm_zetaOrderPLNormalized_le_regularized
...
private theorem zetaOrderPLNormalized_mul_pow
...
/-- Polynomial growth of `ζ` on every finite closed vertical strip. -/
private theorem zeta_vertical_strip_bound_general
    (σ₁ σ₂ : ℝ) (_hσ : σ₁ ≤ σ₂) :
    ∃ A C : ℝ, 0 ≤ A ∧ 0 < C ∧
      ∀ s : ℂ, σ₁ ≤ s.re → s.re ≤ σ₂ → 2 ≤ |s.im| →
        ‖riemannZeta s‖ ≤ C * |s.im| ^ A := by
  ...
  obtain ⟨A, Cₗ, hA, hCₗ, hleft⟩ :=
    riemannZeta_polynomial_bound_of_re_le_neg a a le_rfl ha_neg
  ...
  obtain ⟨Cₐ, hCₐ, hshort_a⟩ :=
    zetaOrderRegularized_bounded_on_short_vertical_line a 2
  obtain ⟨Cb, hCb, hshort_b⟩ :=
    zetaOrderRegularized_bounded_on_short_vertical_line b 2
  ...
  have hboundary_a : ∀ z : ℂ, z.re = a →
    ...
    · calc
        ‖zetaOrderPLNormalized p N z‖ ≤ ‖zetaOrderRegularized z‖ :=
          norm_zetaOrderPLNormalized_le_regularized p N z (by
            dsimp [p]
            linarith)
        _ ≤ Cₐ := hshort_a z hz ht_small
        _ ≤ C₀ := le_max_left _ _
    · have ht : 2 ≤ |z.im| := (not_le.mp ht_small).le
      have hlarge := norm_zetaOrderPLNormalized_on_large_vertical_line
  ...
  have hfd : DiffContOnCl ℂ (zetaOrderPLNormalized p N)
    ...
    zetaOrderPLNormalized_diffContOnCl p a b N (by dsimp [p]; linarith)
  have hgrowth := zetaOrderPLNormalized_vertical_strip_growth
  ...
  have hmul := zetaOrderPLNormalized_mul_pow p N s hsp
  have hreg := zetaOrderRegularized_eq_of_im_ne_zero s him
...
/-- **Convexity bound for ζ in vertical strips.**
    ...
    This follows from the functional equation, complex Stirling bounds, and
    Phragmén–Lindelöf on a larger strip with boundaries left of `0` and right of `1`. -/
theorem zeta_vertical_strip_bound (σ₁ σ₂ : ℝ) (hσ : σ₁ < σ₂)
    (_hstrip : 0 < σ₁ ∨ σ₂ < 1) :
    ∃ (A C : ℝ), 0 < C ∧ ∀ (s : ℂ), σ₁ ≤ s.re → s.re ≤ σ₂ → 2 ≤ |s.im| →
      ‖riemannZeta s‖ ≤ C * |s.im| ^ A := by
  obtain ⟨A, C, _hA, hC, hbound⟩ :=
    zeta_vertical_strip_bound_general σ₁ σ₂ hσ.le
  exact ⟨A, C, hC, hbound⟩
