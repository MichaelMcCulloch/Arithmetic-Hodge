/-
  Closure of Bombieri's linear transpose on the multiplicative test space.

  The raw formula `f*(x) = x⁻¹ f(x⁻¹)` appears singular at zero.  For a test
  function compactly supported inside `(0,∞)`, inversion carries its support
  to another compact subset of `(0,∞)`.  Consequently the transpose is
  identically zero near zero and is globally smooth.
-/
...
/-- Bombieri's transpose of a test function remains compactly supported. -/
theorem transpose_hasCompactSupport (f : BombieriTest) :
...
/-- The topological support of the transpose remains inside `(0,∞)`. -/
theorem transpose_tsupport_subset (f : BombieriTest) :
...
/-- Bombieri's transpose is globally smooth; at zero it vanishes on a whole
neighborhood because its inverted support is compact and positive. -/
theorem transpose_contDiff (f : BombieriTest) :
...
/-- Bombieri's transpose bundled as another positive-half-line test function. -/
def transposeTest (f : BombieriTest) : BombieriTest :=
  TestFunction.mk (transpose (f : ℝ → ℂ)) (transpose_contDiff f)
    (transpose_hasCompactSupport f) (transpose_tsupport_subset f)

@[simp]
theorem transposeTest_apply (f : BombieriTest) (x : ℝ) :
    transposeTest f x = transpose (f : ℝ → ℂ) x := rfl

end BombieriTest

/-- Bombieri's transpose as a genuine complex-linear endomorphism of the test
space. -/
def transposeLinearMap : BombieriTest →ₗ[ℂ] BombieriTest where
  toFun := BombieriTest.transposeTest
  map_add' f g := by
    ext x
    simp only [BombieriTest.transposeTest_apply, TestFunction.coe_add,
      Pi.add_apply]
    simp [transpose, mul_add]
  map_smul' c f := by
    ext x
    simp only [BombieriTest.transposeTest_apply, TestFunction.coe_smul,
      Pi.smul_apply]
    simp [transpose, mul_left_comm]

@[simp]
theorem transposeLinearMap_apply (f : BombieriTest) (x : ℝ) :
    transposeLinearMap f x = transpose (f : ℝ → ℂ) x := rfl

/-- The canonical bundled transpose realization used by the Bombieri explicit
formula. -/
def canonicalTransposeData : TransposeData where
