/-
  Mellin--Fourier infrastructure for Bombieri tests.

  Mathlib's logarithmic change of variables identifies the Mellin transform
  with the Fourier transform of the weighted logarithmic pullback.  Since that
  pullback is Schwartz, vertical Mellin decay, integrability, and inversion are
  unconditional on `BombieriTest`.
-/
...
/-- The Mellin transform is the Fourier transform of the weighted logarithmic
pullback, with Mathlib's `2π` normalization. -/
...
/-- Mellin transforms decay faster than every power on each fixed vertical
line. -/
...
/-- The Mellin transform is integrable on every vertical line. -/
