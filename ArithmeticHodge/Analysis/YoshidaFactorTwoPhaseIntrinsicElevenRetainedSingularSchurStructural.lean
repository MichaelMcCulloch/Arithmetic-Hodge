...
/-- Sharp retuning for the same cutoff-eleven tail reserve and pole row.
The low block retains only `1 / 4096` of the half singular energy, while the
tail retains `1 / 64` and the removed pole row remains `1 / 512`.  These
constants saturate, rather than merely satisfy, the Schur compatibility:
`(1 / 512)^2 = (1 / 4096) * (1 / 64)`. -/
theorem factorTwoEndpointLowTailMixed_centeredPolynomialLift_sq_le_of_retain_one_div_four_thousand_ninety_six
    (pE pO : ℝ[X]) (eR oR : ℝ → ℝ)
    (heRc : Continuous eR) (hoRc : Continuous oR)
    (heR : Function.Even eR) (hoR : Function.Odd oR)
    (heR0 : centeredEvenP0Coefficient eR = 0)
  ...
  exact
    factorTwoEndpointLowTailMixed_sq_le_of_asymmetrically_retained_singular
      ...
      heLowc hoLowc heRc hoRc
      heLowLocal hoLowLocal heRlocal hoRlocal
      a b (1 / 4096 : ℝ) (1 / 64 : ℝ) (1 / 512 : ℝ) lowComplement
      (factorTwoIntrinsicElevenRetainedWeightedReserve eR oR)
      hab (by norm_num) (by norm_num) (by norm_num)
      heRaw hoRaw heOrth hoOrth
      hlowComplement htailComplement hlowRetains htailRetains hremaining
