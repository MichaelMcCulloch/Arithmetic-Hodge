...
/-- The genuinely nonpolynomial Schur residual on the low side.  It keeps
the operator row generated by the canonical finite profile and the endpoint
seed row in their exact coupled combination. -/
def fourCellEvenFiniteSevenSchurResidualRepresenter
    (w : ℝ → ℝ) (hw : Continuous w) (x : ℝ) : ℝ :=
  fourCellEvenFiniteSevenSeedDiagonal *
      fourCellEvenFiniteSevenNonsingularOperatorRepresenter
        (fourCellEvenFiniteSevenCanonicalLow w hw) x -
...
/-- Absorb the fixed endpoint-seed selector into the arbitrary degree-
fourteen quotient selector.  This exposes the Schur residual above with no
separate tail-row representer. -/
def fourCellEvenFiniteSevenSchurResidualSelector
    (w : ℝ → ℝ) (hw : Continuous w) (q : ℝ[X]) : ℝ[X] :=
  fourCellEvenEndpointSeedCanonicalLowThroughTwelveRow w hw •
      fourCellEvenEndpointSeedCanonicalTailSelectorPolynomial + q

theorem natDegree_fourCellEvenFiniteSevenSchurResidualSelector_lt_fourteen
    (w : ℝ → ℝ) (hw : Continuous w) (q : ℝ[X])
    (hq : q.natDegree < 14) :
    (fourCellEvenFiniteSevenSchurResidualSelector w hw q).natDegree < 14 := by
  have hseed :
      (fourCellEvenEndpointSeedCanonicalLowThroughTwelveRow w hw •
        fourCellEvenEndpointSeedCanonicalTailSelectorPolynomial).natDegree <
          14 :=
    (Polynomial.natDegree_smul_le _ _).trans_lt
      (natDegree_fourCellEvenEndpointSeedCanonicalTailSelectorPolynomial_lt_eight
        |>.trans (by norm_num))
  unfold fourCellEvenFiniteSevenSchurResidualSelector
  exact lt_of_le_of_lt (Polynomial.natDegree_add_le _ _) (max_lt hseed hq)

/-- The canonical endpoint tail density is exactly the nonsingular operator
row of the seed modulo its fixed degree-below-eight selector. -/
theorem fourCellEvenEndpointSeedTailFourteenRepresenter_zero_eq_nonsingular_sub_selector
    (x : ℝ) :
    fourCellEvenEndpointSeedTailFourteenRepresenter 0 x =
      fourCellEvenFiniteSevenNonsingularOperatorRepresenter
          fourCellEvenEndpointCoshSeed x -
        centeredPolynomialLift
          fourCellEvenEndpointSeedCanonicalTailSelectorPolynomial x := by
  unfold fourCellEvenEndpointSeedTailFourteenRepresenter
    ...
    fourCellEvenFiniteSevenNonsingularOperatorRepresenter
    fourCellEvenFiniteSevenRegularLagWeight
  rw [← fourCellEvenEndpointCoshSeed_eq_intrinsicEvenP0246Profile]
  simp only [centeredPolynomialLift, Polynomial.eval_zero, sub_zero]
  ring

/-- After the fixed selector is absorbed, the projected bordered density is
pointwise the exact Schur-residual density minus one arbitrary low
polynomial.  This is the structural form whose quotient norm remains. -/
theorem fourCellEvenFiniteSevenProjectedBorderRepresenter_schurResidualSelector_eq
    (w : ℝ → ℝ) (hw : Continuous w) (q : ℝ[X]) (x : ℝ) :
    fourCellEvenFiniteSevenProjectedBorderRepresenter w hw
        (fourCellEvenFiniteSevenSchurResidualSelector w hw q) x =
      fourCellEvenFiniteSevenSchurResidualRepresenter w hw x -
        centeredPolynomialLift q x := by
  unfold fourCellEvenFiniteSevenProjectedBorderRepresenter
    fourCellEvenFiniteSevenExactBorderRepresenter
  rw [fourCellEvenEndpointSeedTailFourteenRepresenter_zero_eq_nonsingular_sub_selector]
  unfold fourCellEvenFiniteSevenSchurResidualSelector
    fourCellEvenFiniteSevenSchurResidualRepresenter
    centeredPolynomialLift
...
/-- The mixed scalar is the tail pairing of the coupled Schur-residual
density itself.  The fixed endpoint-row selector has disappeared completely;
only an optional additional degree-below-fourteen quotient remains. -/
theorem fourCellEvenFiniteSevenExactBorderMixed_eq_schurResidualPairing
    (w : ℝ → ℝ) (hw : Continuous w)
    (hlocal : LocallyLipschitzOn (Icc (-1 : ℝ) 1) w)
    (heven : Function.Even w)
    (q : ℝ[X]) (hq : q.natDegree < 14) :
    fourCellEvenFiniteSevenExactBorderMixed w hw =
      ∫ x : ℝ in -1..1,
        (fourCellEvenFiniteSevenSchurResidualRepresenter w hw x -
          ...
          fourCellEvenFiniteSevenCanonicalTail w hw x := by
  let Q : ℝ[X] :=
    fourCellEvenFiniteSevenSchurResidualSelector w hw q
  have hQ : Q.natDegree < 14 := by
    simpa only [Q] using
      natDegree_fourCellEvenFiniteSevenSchurResidualSelector_lt_fourteen
        w hw q hq
  have hpair :=
  ...
  change
    fourCellEvenFiniteSevenProjectedBorderRepresenter w hw Q x *
        fourCellEvenFiniteSevenCanonicalTail w hw x =
      (fourCellEvenFiniteSevenSchurResidualRepresenter w hw x -
          centeredPolynomialLift q x) *
        fourCellEvenFiniteSevenCanonicalTail w hw x
  rw [show Q = fourCellEvenFiniteSevenSchurResidualSelector w hw q by rfl,
    fourCellEvenFiniteSevenProjectedBorderRepresenter_schurResidualSelector_eq]

/-- In particular, no selector is needed to state the exact residual pairing;
further selectors only lower the quotient norm used in Cauchy's inequality. -/
theorem fourCellEvenFiniteSevenExactBorderMixed_eq_schurResidualPairing_zero
    (w : ℝ → ℝ) (hw : Continuous w)
    (hlocal : LocallyLipschitzOn (Icc (-1 : ℝ) 1) w)
    (heven : Function.Even w) :
    fourCellEvenFiniteSevenExactBorderMixed w hw =
      ∫ x : ℝ in -1..1,
        fourCellEvenFiniteSevenSchurResidualRepresenter w hw x *
          fourCellEvenFiniteSevenCanonicalTail w hw x := by
  simpa only [centeredPolynomialLift, Polynomial.eval_zero, sub_zero] using
    fourCellEvenFiniteSevenExactBorderMixed_eq_schurResidualPairing
      w hw hlocal heven 0 (by norm_num)
...
/-- A successful residual quotient-norm estimate already certifies the
finite low diagonal: its left side is a squared `L²` norm and its scalar
coefficient is strictly positive.  Thus low positivity is not an additional
hypothesis in the residual formulation. -/
theorem fourCellEvenFiniteSevenExactLowDiagonal_nonnegative_of_schurResidualNorm
    (w : ℝ → ℝ) (hw : Continuous w) (q : ℝ[X])
    (hnorm :
      (∫ x : ℝ in -1..1,
        (fourCellEvenFiniteSevenSchurResidualRepresenter w hw x -
          ...
          ((49 / 10000 : ℝ) * (harmonic 14 : ℝ)) *
            fourCellEvenFiniteSevenExactLowDiagonal w hw) :
    0 ≤ fourCellEvenFiniteSevenExactLowDiagonal w hw := by
  have hnormNonneg : 0 ≤ ∫ x : ℝ in -1..1,
      (fourCellEvenFiniteSevenSchurResidualRepresenter w hw x -
        centeredPolynomialLift q x) ^ 2 :=
    intervalIntegral.integral_nonneg (by norm_num) (fun _ _ ↦ sq_nonneg _)
  have hconstant : 0 <
      (49 / 10000 : ℝ) * (harmonic 14 : ℝ) := by
    norm_num [harmonic, Finset.sum_range_succ]
  apply (mul_nonneg_iff_of_pos_left hconstant).mp
  exact hnormNonneg.trans hnorm

/-- Selector-free formulation of the same determinant reduction.  The only
remaining analytic estimate is now visibly the quotient norm of the coupled
Schur-residual density, rather than a norm containing the endpoint tail row
as a separate summand. -/
theorem fourCellEvenFiniteSevenExactBorderMixed_sq_le_of_schurResidualNorm
    (w : ℝ → ℝ) (hw : Continuous w)
    (hlocal : LocallyLipschitzOn (Icc (-1 : ℝ) 1) w)
    (heven : Function.Even w)
    (q : ℝ[X]) (hq : q.natDegree < 14)
  ...
  have hQ : Q.natDegree < 14 := by
    simpa only [Q] using
      natDegree_fourCellEvenFiniteSevenSchurResidualSelector_lt_fourteen
        w hw q hq
  have hlow :=
    fourCellEvenFiniteSevenExactLowDiagonal_nonnegative_of_schurResidualNorm
      w hw q hnorm
  have hnorm' :
      (∫ x : ℝ in -1..1,
        fourCellEvenFiniteSevenProjectedBorderRepresenter w hw Q x ^ 2) ≤
...
/-- Minimal structural endpoint closure.  A single degree-below-fourteen
selector and its coupled Schur-residual quotient-norm bound imply both low
diagonal positivity and the exact mixed determinant, hence the endpoint row.
No finite entry box or separate synthesis hypothesis is present here. -/
theorem fourCellEvenEndpointSeedRow_sq_le_seed_mul_polarFree_of_schurResidualNorm
    (w : ℝ → ℝ) (hw : Continuous w)
    (hlocal : LocallyLipschitzOn (Icc (-1 : ℝ) 1) w)
    (heven : Function.Even w)
    (hzero : fourCellPositiveCoshMoment w
  ...
  have hlow :=
    fourCellEvenFiniteSevenExactLowDiagonal_nonnegative_of_schurResidualNorm
      w hw q hnorm
  have hmixed :=
    fourCellEvenFiniteSevenExactBorderMixed_sq_le_of_schurResidualNorm
      w hw hlocal heven q hq hnorm
  exact
