...
import ArithmeticHodge.Analysis.YoshidaRegularKernelMeanZeroSchur

set_option autoImplicit false

open Complex MeasureTheory Real Set

namespace ArithmeticHodge.Analysis.YoshidaRegularKernelSharpMeanZeroSchur
...
/-!
# Sharp mean-zero Schur bound for Yoshida's regular kernel

The structural range `7 / 32 ≤ h ≤ 1 / 4` puts the regular kernel
within `1 / 64` of its midpoint `15 / 64`.  On the mean-zero subspace
that constant component vanishes exactly, giving the improved endpoint
quadratic estimate without a mode cutoff or numerical certificate.
-/

/-- Centering the regular kernel at `15 / 64` and cancelling that constant on
the mean-zero subspace improves its endpoint quadratic bound to `1 / 32`. -/
theorem norm_yoshidaEndpointRegularQuadratic_le_one_thirty_second_of_integral_eq_zero
  ...
  have hdomInt : Integrable
      (fun p : ℝ × ℝ ↦ (1 / 64 : ℝ) * (‖f p.1‖ * ‖f p.2‖))
      (μ.prod μ) :=
    hprodNorm.const_mul (1 / 64 : ℝ)
  ...
  have hCInt : Integrable C (μ.prod μ) := by
    dsimp only [C]
    exact hstarProdInt.const_mul (15 / 64 : ℂ)
  ...
  have hCzero : (∫ p : ℝ × ℝ, C p ∂μ.prod μ) = 0 := by
    ...
    calc
      (∫ p : ℝ × ℝ,
          (15 / 64 : ℂ) * (star (f p.1) * f p.2) ∂μ.prod μ) =
        (15 / 64 : ℂ) *
  ...
  have hquad₀ :
      ‖∫ p : ℝ × ℝ, G₀ p ∂μ.prod μ‖ ≤
        (1 / 64 : ℝ) * (∫ x : ℝ, ‖f x‖ ∂μ) ^ 2 := by
    calc
      ‖∫ p : ℝ × ℝ, G₀ p ∂μ.prod μ‖ ≤
          ∫ p : ℝ × ℝ, ‖G₀ p‖ ∂μ.prod μ :=
        norm_integral_le_integral_norm _
      _ ≤ ∫ p : ℝ × ℝ,
          (1 / 64 : ℝ) * (‖f p.1‖ * ‖f p.2‖) ∂μ.prod μ := by
...
end ArithmeticHodge.Analysis.YoshidaRegularKernelSharpMeanZeroSchur
