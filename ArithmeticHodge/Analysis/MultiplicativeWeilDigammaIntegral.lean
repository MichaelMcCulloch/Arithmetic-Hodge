/-
  Integral forms of Bombieri's digamma kernel.

  Bombieri's equations (2.5)--(2.6) pass from the real part of the digamma
  expansion to Cauchy-kernel averages of the Mellin transform.  The results
  below make that passage explicit, including the harmonic subtraction and
  the normalized Mellin inversion term at `x = 1`.
-/
...
/-- A Bombieri digamma summand is a Cauchy kernel, so its normalized vertical
Mellin integral is the Mellin transform after multiplication by the positive
weight with parameter `2k + 1/2`.  This is the kernel integral used between
Bombieri's equations (2.5) and (2.6). -/
theorem bombieriDigammaKernel_integral
    (f : BombieriTest) (k : ℕ) :
    ((1 / (2 * Real.pi) : ℝ) : ℂ) *
        ∫ v : ℝ,
          (bombieriDigammaKernel k v : ℂ) *
...
/-- The digamma Cauchy kernel times a critical-line Bombieri--Mellin
transform is Bochner integrable.  Thus the normalized integral in
`bombieriDigammaKernel_integral` is not relying on totalization. -/
theorem bombieriDigammaKernel_mul_mellin_integrable
    (f : BombieriTest) (k : ℕ) :
    Integrable (fun v : ℝ ↦
      (bombieriDigammaKernel k v : ℂ) *
...
/-- Normalized Mellin inversion on the critical line at `x = 1`.  This is
the constant harmonic-subtraction term in Bombieri's equation (2.6). -/
theorem normalized_integral_mellin_eq_apply_one
    (f : BombieriTest) :
    ((1 / (2 * Real.pi) : ℝ) : ℂ) *
...
/-- The normalized integral of a digamma kernel after subtracting its
harmonic counterterm.  This is the individual summand identity underlying
the series transition in Bombieri's equations (2.5)--(2.6). -/
theorem bombieriDigammaKernel_sub_integral
    (f : BombieriTest) (n : ℕ) :
    ((1 / (2 * Real.pi) : ℝ) : ℂ) *
        ∫ v : ℝ,
