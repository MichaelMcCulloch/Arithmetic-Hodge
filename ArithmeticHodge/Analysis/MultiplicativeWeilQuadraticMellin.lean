/-
  Absolute Mellin convergence for Bombieri's quadratic convolution.

  Compact support away from both coordinate axes makes the two-variable
  Mellin kernel genuinely Bochner integrable for every complex argument.
-/
...
private theorem convolutionMellinKernel_eq_zero_of_pos_notMem
  ...
  have hinv : g p.2⁻¹ = 0 := by
    ...
    have hp2 : 0 < p.2 := hp.2
    refine ⟨(p.1 / p.2, p.2⁻¹),
      ⟨subset_tsupport g ?_, subset_tsupport g ?_⟩, ?_⟩
...
/-- The quadratic Mellin kernel is absolutely integrable at every complex
argument. -/
theorem bombieriQuadraticConvolutionFubiniAt
    (g : BombieriTest) (s : ℂ) :
...
/-- The bundled quadratic test has the expected Mellin spectral factor at
  ...
  simpa only [mellin_transposeConjugate, spectralTerm] using
    hasMellin_convolution (g : ℝ → ℂ)
      ...
      ⟨g.mellinConvergent s, htgc,
        bombieriQuadraticConvolutionFubiniAt g s⟩
