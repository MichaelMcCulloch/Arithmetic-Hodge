/-
  Weighted principal-part removal on closed complex rectangles.

  Multiplying a logarithmic derivative by a holomorphic weight changes the
  residue at `ρ` from `ord(ρ)` to `ord(ρ) H(ρ)`.  Subtracting those
  weighted simple poles leaves a meromorphic function with nonnegative order
  throughout the rectangle; the local cancellation is expressed by `dslope`.
-/
