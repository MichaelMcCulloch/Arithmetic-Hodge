...
/-!
# Rectangular Cauchy integral formula

This file evaluates the rectangular contour integral of `H(s) / (s - ρ)` when
`H` is holomorphic on a neighborhood of the closed axis-aligned rectangle and
`ρ` lies strictly inside it.  The proof combines the removable `dslope`
quotient with rectangular Cauchy–Goursat and the simple-pole winding integral.
-/
...
/-- **Rectangular Cauchy integral formula.** If `H` is complex differentiable
at every point of the closed rectangle and `rho` lies strictly inside, then
integrating `H(s) / (s - rho)` around the rectangle gives `2πi H(rho)`. -/
theorem rectIntegral_cauchy_kernel
    {H : ℂ → ℂ} {z w rho : ℂ}
