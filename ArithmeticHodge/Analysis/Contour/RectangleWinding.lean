...
/-!
# Rectangular winding integral

This file supplies the rectangular simple-pole winding/residue bridge.  It
evaluates the repository's axis-aligned `rectIntegral` of `(z - ρ)⁻¹` when the
pole `ρ` lies strictly inside the rectangle.

The proof uses principal-log primitives on each edge.  On the left edge it
uses `log (-z)` instead of `log z`, keeping that primitive off the principal
logarithm's branch cut.  The endpoint logarithms then cancel, leaving the two
argument jumps that contribute `2πi`.
-/
...
/--
The rectangular simple-pole winding/residue bridge: if `ρ` lies strictly
inside the axis-aligned rectangle with opposite corners `z` and `w`, then the
rectangular contour integral of `(s - ρ)⁻¹` is `2πi`.
-/
theorem rectIntegral_sub_inv_of_mem_rectangle
    {z w ρ : ℂ}
