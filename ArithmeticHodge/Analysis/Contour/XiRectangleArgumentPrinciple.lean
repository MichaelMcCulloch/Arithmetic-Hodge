...
theorem rectIntegral_xi_logDeriv
  ...
  have hsupp : ∀ s ∈ ([[z.re, w.re]] ×ℂ [[z.im, w.im]]),
    ...
    intro s hs horder
    exact xi_meromorphicOrderAt_ne_zero_mem_xiZerosInRectangle
      z.re w.re z.im w.im
      (by simpa only [xiZeroRectangle, uIcc_of_le hrew.le,
        uIcc_of_le himw.le] using hs) horder
  have hord := xiZerosInRectangle_meromorphicOrderAt_eq_multiplicity
    z.re w.re z.im w.im
...
/-- Boundary nonvanishing supplies the strict-interior hypothesis in the xi
rectangle argument principle. -/
theorem rectIntegral_xi_logDeriv_of_boundary_zero_free
    {H : ℂ → ℂ} {z w : ℂ}
    (hrew : z.re < w.re) (himw : z.im < w.im)
    (hH : AnalyticOnNhd ℂ H ([[z.re, w.re]] ×ℂ [[z.im, w.im]]))
    ...
    rectIntegral (fun s => H s * logDeriv xiFunction s) z w =
      2 * (Real.pi : ℂ) * I *
        ∑ rho ∈ xiZerosInRectangle z.re w.re z.im w.im,
          (xiZeroMultiplicity rho : ℂ) * H rho := by
  exact rectIntegral_xi_logDeriv hrew himw hH
    (xiZerosInRectangle_strictInterior_of_boundary_zero_free
      z.re w.re z.im w.im hboundary)
