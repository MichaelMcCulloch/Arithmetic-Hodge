...
theorem log_add_two_pow_le_const_mul_sqrtScratch
    (m : ℕ) {x : ℝ} (hx : 1 ≤ x) :
    Real.log (x + 2) ^ m ≤
...
`3/2` power once `x ≥ 1`. -/
theorem log_add_two_pow_mul_inv_sq_leScratch
    (m : ℕ) {x : ℝ} (hx : 1 ≤ x) :
    Real.log (x + 2) ^ m * x⁻¹ ^ 2 ≤
      ((2 * (m + 1 : ℝ)) ^ m * 3 ^ (1 / 2 : ℝ)) *
        x⁻¹ ^ (3 / 2 : ℝ) := by
  have hx0 : 0 < x := zero_lt_one.trans_le hx
  have hlog := log_add_two_pow_le_const_mul_sqrtScratch m hx
...
theorem ZetaZeroEnumeration.summable_log_norm_add_two_pow_mul_inv_sqScratch
  ...
  simpa only [a, p] using log_add_two_pow_mul_inv_sq_leScratch m hk
